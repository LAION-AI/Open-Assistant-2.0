...
func TestIngestHandlerEndToEnd(t *testing.T) {
	...
	h(rr, req)
	if rr.Code != http.StatusOK { t.Fatalf("want 200, got %d: %s", rr.Code, rr.Body) }
	var res struct{ Saved int `json:"saved"` }
	json.Unmarshal(rr.Body.Bytes(), &res)
	if res.Saved != 1 { t.Fatalf("want saved=1, got %d", res.Saved) }
	...
	rr2 := httptest.NewRecorder(); h(rr2, req2)
	if rr2.Code != http.StatusOK { t.Fatalf("re-upload want 200, got %d", rr2.Code) }

	// bad key -> 401
	reqBad := httptest.NewRequest("POST", "/api/ingest", strings.NewReader(body))
	reqBad.Header.Set("Authorization", "Bearer oa-wrong")
	rrBad := httptest.NewRecorder(); h(rrBad, reqBad)
	if rrBad.Code != http.StatusUnauthorized { t.Fatalf("bad key want 401, got %d", rrBad.Code) }

	// missing key -> 401
	reqNo := httptest.NewRequest("POST", "/api/ingest", strings.NewReader(body))
	rrNo := httptest.NewRecorder(); h(rrNo, reqNo)
	if rrNo.Code != http.StatusUnauthorized { t.Fatalf("missing key want 401, got %d", rrNo.Code) }
}
